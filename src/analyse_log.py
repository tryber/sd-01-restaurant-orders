...
# Código feito com ajuda do Victor Felipe e Gabriel Coruja - https://github.com/tryber/sd-01-restaurant-orders/tree/gabrielcoruja-restaurant-orders
...
def analyse_log(path_to_file, plate_favorite_customer, order, plates_orders_customer, absent_person):

    filter_plate_favorite = read_file_csv(path_to_file, plate_favorite_customer)
    filter_quantity_order = read_file_csv(path_to_file, order[0])
    filter_plates_favorites = read_file_csv(path_to_file, plates_orders_customer)
    ...
    with open('../data/mkt_campaign.txt', 'w', newline='') as file:
        ...
        file.write(f"O(A) {plates_orders_customer} nunca pediu: {plates_not_orders}\n")
