...
# Código feito com ajuda do Guilherme Luis e Gabriel Coruja
# https://github.com/tryber/sd-01-restaurant-orders/tree/guiiluiz-restaurant-orders
...
def quantity_orders_food(foods, food_choose):
...
def analyse_log(
        path_to_file,
        plate_favorite_customer,
        order,
        plates_orders_customer,
        absent_person):

    filter_plate_favorite = read_file_csv(
        path_to_file, plate_favorite_customer)
    filter_quantity_order = read_file_csv(path_to_file, order[0])
    filter_plates_favorites = read_file_csv(
        path_to_file, plates_orders_customer)
    filter_present_days = read_file_csv(path_to_file, absent_person)

    food_more_order = favorite_food(filter_plate_favorite)
    food_order_total = quantity_orders_food(filter_quantity_order, order[1])
    ...
    with open('../data/mkt_campaign.txt', 'w', newline='') as file:
        ...
        file.write(f"""O prato mais pedido por
         {plate_favorite_customer}: {food_more_order}\n""")
        file.write(f"""A quantidade de vezes que
         {order[0]} pediu {order[1]} foi: {food_order_total}\n""")
        file.write(f"""O(A) {plates_orders_customer}
         nunca pediu: {plates_not_orders}\n""")
        file.write(f"""O(A) {absent_person}
         nunca foi na lanchonete no(s) dia(s): {days_not_present}\n""")


analyse_log(
    '../data/orders_1.csv', "maria", ["arnaldo", "hamburguer"], "joao", "joao")
