...
# Parte do código feito com ajuda do Doug Funny e Guilherme Luis
...
maria,pizza,terça-feira"""
    ...
    def __reading_the_main_file(self):
        ...
        with patch("builtins.open", open_mock):
            ...
            for k in new_array:
                costumer, order, day = k.get(
                  "costumer"), k.get("order"), k.get("day")
    ...
    def test_get_order_frequency_per_costumer_true(self):
        self.__reading_the_main_file()
        expected = self.tracker.get_order_frequency_per_costumer(
          "arnaldo", "hamburguer")
        received = """Quantidade de pedidos de
         hamburguer feito pelo/a cliente Arnaldo: 32"""
        assert expected == received
    ...
    def test_get_days_most_visited_per_costumer(self):
        ...
        received = """Dia(s) mais visitado(s) pelo(a) cliente joao:
         {'terça-feira', 'sabado', 'segunda-feira'}"""
        print("#-" * 45)
        print(expected)
        print("#-" * 45)
        assert expected == received
